export class Kata {
    ...
    public solveKnightsTour(startX: number, startY: number): number[][][] {
        ...
        this.knightsTourHelper(board, startX, startY, 1, allSolutions);

        return allSolutions;
    }

    private knightsTourHelper(
        board: number[][],
        x: number,
        y: number,
        moveCount: number,
        ...
        if (moveCount === this.boardSize * this.boardSize) {
            // Clone the solution and add to the list
        ...
        for (const [nextX, nextY] of moves) {
            ...
            this.knightsTourHelper(board, nextX, nextY, moveCount + 1, allSolutions);
    ...
    public displayBoard(board: number[][]): string {
        return board
            .map(row => row.map(cell => cell.toString().padStart(2, '0')).join(' '))
            .join('\n');
    }

    public displaySolutions(solutions: number[][][]): void {
        if (!solutions.length) {
        ...
        solutions.forEach((solution, index) => {
            ...
            console.log(this.displayBoard(solution));
            console.log('');
        });
    }
}
